...
var Crawler = function(host,initialPath,initialPort,interval) {
	...
	crawler.useDefaultDiscovery = true;
...
Crawler.prototype.discoverResources = function(resourceData,queueItem) {
	...
	if(crawler.useDefaultDiscovery === true) {
		// Rough scan for URLs
	...
	} else if (typeof (crawler.useDefaultDiscovery) === "function") {
		return cleanAndQueue(crawler.useDefaultDiscovery(resourceText, queueItem));
	} else {
		return [];
	}
};
	...
	if (crawler.domainValid(parsedURL.host)) {
		crawler.queue.add(
			...
			function queueAddCallback(error,newQueueItem) {
				...
				newQueueItem.referrer = queueItem ? queueItem.url : "";
			}
		);
	}
